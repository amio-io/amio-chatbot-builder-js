...
class Chatbot {
  ...
  async runPostback(webhook) {
    const {channelId, contactId, data} = webhook
    const payload = path(['postback', 'payload'], data)
    ...
    await this.runNextState(channelId, contactId, data)
  }

  async _runInterceptors(webhook, method) {
    const {channelId, contactId, data} = data
    try {
      let shallContinue
  ...
  async _resolveNextStateFromLastState(webhook) {
    ...
    if (lastState) {
      ...
      return await lastState.findNextState(channelId, contactId, data, lastState)
    }
